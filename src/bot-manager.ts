...
export class BotManager extends EventEmitter {
  ...
  private getBriefStatus(username: string): string {
    ...
    for (const entity of Object.values(bot.entities)) {
      ...
      if (dist < 12 && hostileMobs.includes(entity.name?.toLowerCase() || "")) {
        dangers.push(`${entity.name}(${dist.toFixed(0)}m)`);
      }
    }

    const blockLight = bot.blockAt(pos)?.light ?? 0;
    if (blockLight < 8) dangers.push(`暗(光${blockLight})`);
  ...
  getSurroundings(username: string): string {
    ...
    // 光レベル（モブスポーン判定）
    const blockLight = bot.blockAt(pos)?.light ?? 0;
    const lightWarning = blockLight < 8 ? " ⚠️モブスポーン危険" : "";
    lines.push(`光レベル: ${blockLight}${lightWarning}`);
  ...
  async digBlock(username: string, x: number, y: number, z: number, useCommand: boolean = false): Promise<string> {
    ...
    try {
      ...
      // If nothing picked up yet, try moving closer to block position
      if (pickedUp === 0) {
        // Walk directly to block position (not y-1)
        const goal = new goals.GoalNear(Math.floor(x), Math.floor(y), Math.floor(z), 1);
        bot.pathfinder.setGoal(goal);
        await this.delay(1000);
        bot.pathfinder.setGoal(null);

        // Wait a bit more for auto-pickup
        await this.delay(500);

        // Check again
        inventoryAfter = bot.inventory.items().reduce((sum, i) => sum + i.count, 0);
        pickedUp = inventoryAfter - inventoryBefore;
        console.error(`[Dig] Inventory check 2: before=${inventoryBefore}, after=${inventoryAfter}, picked=${pickedUp}`);
      }
      ...
      if (droppedItems.length > 0) {
        return `Dug ${blockName} - ${droppedItems.length} item(s) nearby. Try collect_items.` + this.getBriefStatus(username);
      } else {
        ...
        return `Dug ${blockName} with ${heldItem}.` + this.getBriefStatus(username);
      }
    } catch (err) {
