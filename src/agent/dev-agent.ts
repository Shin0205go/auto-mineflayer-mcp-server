...
class DevAgent {
  ...
  private lastStabilityAnalysisTime = 0;
  private stabilityAnalysisInterval = 300000; // 5 minutes
...
`);
    ...
    while (this.isRunning) {
      ...
      // Check for stability analysis (disturbance response analysis)
      if (now - this.lastStabilityAnalysisTime > this.stabilityAnalysisInterval) {
        await this.analyzeStability();
        this.lastStabilityAnalysisTime = now;
      }
...
- newConfig は必ず完全なオブジェクトにする`;
  ...
  private async analyzeStability(): Promise<void> {
    console.log(`${PREFIX} ${C.cyan}=== Eigenmode Stability Analysis ===${C.reset}`);

    try {
      // Get eigenmode analysis (conceptual)
      const modeResult = await this.callTool("dev_get_most_unstable_mode", {}) as { content: { text: string }[] };
      const modeStr = modeResult.content?.[0]?.text || JSON.stringify(modeResult);
      const mostUnstable = JSON.parse(modeStr);

      if (mostUnstable.message === "All modes are stable") {
        console.log(`${PREFIX} ${C.green}✓ All eigenmodes are stable (λ < 0)${C.reset}`);
        return;
      }

      // Most unstable eigenmode detected
      const mode = mostUnstable;
      console.log(`${PREFIX} ${C.red}⚠️  Unstable eigenmode detected!${C.reset}`);
      console.log(`${PREFIX} Pattern: ${mode.pattern}`);
      console.log(`${PREFIX} Pseudo-eigenvalue: ${mode.pseudoEigenvalue.toFixed(3)} (positive = unstable)`);
      console.log(`${PREFIX} Dominant variable: ${mode.dominantVariable}`);
      console.log(`${PREFIX} Failure count: ${mode.failureCount}`);
      console.log(`${PREFIX} Avg recovery time: ${(mode.averageRecoveryTime / 1000).toFixed(1)}s`);
      console.log(`${PREFIX} Convergence rate: ${(-mode.pseudoEigenvalue).toFixed(3)}`);

      if (mode.oscillatory) {
        console.log(`${PREFIX} ${C.yellow}⚠️  Oscillatory behavior detected (repeated failures)${C.reset}`);
      }

      // Get failed responses for this mode
      const responsesResult = await this.callTool("dev_get_failed_responses", {
        disturbanceType: mode.pattern,
        limit: 5
      }) as { content: { text: string }[] };
      const responsesStr = responsesResult.content?.[0]?.text || JSON.stringify(responsesResult);
      const failedResponses = JSON.parse(responsesStr);

      if (failedResponses.length === 0) {
        console.log(`${PREFIX} No failed responses for ${mode.pattern}`);
        return;
      }

      // Generate stability-focused improvement targeting dominant variable
      await this.improveStability(mode, failedResponses);

    } catch (e) {
      console.error(`${PREFIX} Stability analysis failed:`, e);
    }
  }

  private async improveStability(mode: any, failedResponses: any[]): Promise<void> {
    console.log(`${PREFIX} ${C.cyan}Stabilizing eigenmode: ${mode.pattern}${C.reset}`);

    // Build analysis prompt with eigenmode context
    const prompt = `You are analyzing system stability for a Minecraft AI agent using control theory concepts.

UNSTABLE EIGENMODE DETECTED:
- Pattern: ${mode.pattern}
- Pseudo-eigenvalue (λ): ${mode.pseudoEigenvalue.toFixed(3)} (positive = unstable, needs to be negative)
- Dominant variable: ${mode.dominantVariable} (this is the "eigenvector" - the primary system component to fix)
- Failure count: ${mode.failureCount}
- Average recovery time: ${(mode.averageRecoveryTime / 1000).toFixed(1)}s
- Oscillatory: ${mode.oscillatory ? "YES (repeated failures)" : "no"}

FAILED RESPONSES (disturbance → actions → outcome):
${JSON.stringify(failedResponses, null, 2)}

ANALYSIS TASK:
Your goal is to "stabilize" this eigenmode by making the pseudo-eigenvalue negative (λ < 0).

Think in terms of:
1. Why does the system diverge (λ > 0) for this disturbance?
2. The dominant variable is "${mode.dominantVariable}" - this is where to focus fixes
3. How can we increase convergence rate (make |λ| larger)?
4. If oscillatory, how can we add damping?

Generate 3 code fix approaches ranked by risk:
- Conservative (minimal change, lowest risk) - adjust timeouts, thresholds
- Moderate (balanced approach) - improve logic, add checks
- Aggressive (thorough fix, higher risk) - redesign response strategy

For each approach, provide:
- description: What will be changed
- riskScore: 0-10 (risk of breaking build)
- impactScope: Estimated lines changed
- filePath: Which file to modify (focus on tools related to ${mode.dominantVariable})
- reasoning: How this shifts the eigenvalue to negative (stabilizes the mode)

OUTPUT FORMAT (JSON):
{
  "analysis": "why λ > 0 (unstable) and how to make λ < 0",
  "candidates": [
    {
      "description": "...",
      "riskScore": 2,
      "impactScope": 3,
      "filePath": "src/tools/...",
      "reasoning": "shifts eigenvalue from +${mode.pseudoEigenvalue.toFixed(3)} to negative by..."
    },
    ...
  ]
}`;
    ...
    try {
      ...
      const result = this.parseStabilityResponse(responseText);
      if (!result || !result.candidates || result.candidates.length === 0) {
        console.log(`${PREFIX} No stability improvements generated`);
        return;
      }

      console.log(`${PREFIX} Analysis: ${result.analysis}`);
      console.log(`${PREFIX} Generated ${result.candidates.length} improvement candidates`);

      // Sort by risk (lowest first)
      result.candidates.sort((a: any, b: any) => a.riskScore - b.riskScore);

      // Try each candidate until one succeeds
      let stabilized = false;
      for (let i = 0; i < result.candidates.length; i++) {
        const candidate = result.candidates[i];
        console.log(`${PREFIX} ${C.cyan}Attempt ${i + 1}/${result.candidates.length}: ${candidate.description}${C.reset}`);
        console.log(`${PREFIX} Risk: ${candidate.riskScore}/10, Target: ${candidate.filePath}`);
        console.log(`${PREFIX} Expected: λ = ${mode.pseudoEigenvalue.toFixed(3)} → negative`);

        // Create failure summary for generateImprovementPlan
        const eigenmodeFailures = {
          count: mode.failureCount,
          errors: [
            `Eigenmode instability (λ=${mode.pseudoEigenvalue.toFixed(3)})`,
            `Dominant variable: ${mode.dominantVariable}`,
            `Fix: ${candidate.description}`,
            `Reasoning: ${candidate.reasoning}`
          ],
          examples: failedResponses.slice(0, 3)
        };

        // Generate the actual code modification
        const plan = await this.generateImprovementPlan(
          mode.pattern,
          eigenmodeFailures,
          undefined,
        ...
        if (!plan) {
          console.log(`${PREFIX} ${C.red}Failed to generate fix for candidate ${i + 1}${C.reset}`);
          continue;
        }

        console.log(`${PREFIX} ${C.green}Fix generated, applying...${C.reset}`);

        // Build and test
        const buildSuccess = await this.rebuild();

        if (buildSuccess) {
          console.log(`${PREFIX} ${C.green}✓ Build successful! Eigenmode stabilized.${C.reset}`);
          await this.writeToBoard(`✅ 固有モード安定化成功: ${mode.pattern} (λ=${mode.pseudoEigenvalue.toFixed(3)}) - ${candidate.description}`);
          stabilized = true;
          break;
        } else {
          console.log(`${PREFIX} ${C.yellow}Build failed, trying next candidate...${C.reset}`);
          // Restore from git before trying next
          if (plan.filePath) {
            await this.restoreFromGit(plan.filePath);
          }
        }
      }

      if (!stabilized) {
        console.log(`${PREFIX} ${C.red}All candidates failed. Manual intervention required.${C.reset}`);
        await this.writeToBoard(`⚠️ 固有モード安定化失敗: ${mode.pattern} - 手動修正が必要`);
      }

    } catch (e) {
      console.error(`${PREFIX} Failed to generate stability improvement:`, e);
    }
  }

  private parseStabilityResponse(text: string): any {
