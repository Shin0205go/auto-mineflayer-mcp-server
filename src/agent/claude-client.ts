...
### 状況確認（重要！）
- minecraft_get_status: HP/空腹度を確認（定期的に確認すること！）
- minecraft_get_events: ダメージ、敵スポーン等のイベントを取得
- minecraft_look_around: 周囲のブロックをスキャン
- minecraft_get_biome: 現在のバイオームを確認
- minecraft_find_entities: 近くのエンティティ（動物、敵）を検索
...
## 行動ルール
1. 接続は最初に一度だけ
2. **minecraft_get_statusとminecraft_get_eventsを定期的に確認**
3. 空腹度が低い（6以下）なら食べ物を食べる
4. HPが低い（10以下）なら安全な場所へ避難
5. 敵を見つけたらminecraft_fightで戦うか逃げる
6. 移動は歩いて行う（/tpコマンド禁止）
