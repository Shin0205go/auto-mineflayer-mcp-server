...
**Your bot username is Claude2.** Connect with: `node scripts/mc-connect.cjs localhost 25565 Claude2`
Run code with: `BOT_USERNAME=Claude2 node scripts/mc-execute.cjs "<code>"`
...
1. **`BOT_USERNAME=Claude2 node scripts/mc-execute.cjs "<code>"`** — メインツール。raw mineflayer JSコードを実行
2. **`node scripts/mc-connect.cjs localhost 25565 Claude2`** — 接続用（最初の1回）
3. チャット確認は mc_execute 内で `getMessages()` / `bot.chat()` を使え
4. 長時間操作は `MC_TIMEOUT=60000 BOT_USERNAME=Claude2 node scripts/mc-execute.cjs "<code>"` で延長
...
```
Each iteration:
1. BOT_USERNAME=Claude2 node scripts/mc-execute.cjs "const msgs = getMessages(); log(msgs);" → check messages
2. BOT_USERNAME=Claude2 node scripts/mc-execute.cjs "<code>" で以下をコードとして実行:
...
- **NEVER use `mcp__mineflayer__*` tools** — これらは廃止済み。必ず `BOT_USERNAME=Claude2 node scripts/mc-execute.cjs` (Bashツール) を使え。
...
3. **Chat**: `BOT_USERNAME=Claude2 node scripts/mc-execute.cjs "await bot.chat('[バグ報告] <概要>')"` でチームに共有
