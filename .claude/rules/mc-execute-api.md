...
await waitForChunksToLoad(timeoutMs?)         // 注入済み: /tp後やディメンション変更後にチャンク読み込み完了を待つ。bot.blockAt()がnullを返す場合はこれを呼ぶ。{ loaded, blockAtFeet, waitedMs } を返す。デフォルト10秒タイムアウト
await resyncPhysics(timeoutMs?)               // 注入済み: サーバーのラバーバンド（位置凍結）を解除。/tp後にbot.setControlState/pathfinderが動かない場合に呼ぶ。position_lookパケットを送信してforcedMoveイベントを待つ。{ synced, waitedMs, position } を返す。デフォルト3秒タイムアウト
