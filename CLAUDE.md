...
**高レベルツールを必ず優先。低レベルツールは最終手段。**

| やりたいこと | 使うもの |
|---|---|
| 食料・HP回復 | `mc_eat()` / `mc_combat(target="cow")` |
| 素材収集 | `mc_gather(block=..., count=N)` |
| 道具作成 | `mc_craft(item=..., autoGather=true)` |
| 建築 | `mc_build(preset=..., size=...)` |
| 探索 | `mc_navigate(target_block=...)` |
| チーム連携 | `mc_chat()` 毎アクション確認（最優先） |

詳細手順: `.claude/skills-compact/` 参照
...
- **リスポーンでHP回復するな。** 食料を食べろ。食料がなければ `mc_combat(target="cow")` で確保。
- **adminの/giveに頼るな。** 全アイテム自力入手。
- 同じ行動を3回失敗したらアプローチを変えろ。
- **tmp_scripts/ にスクリプトを作るな。** MCPツール（`mc_navigate`, `mc_gather` 等）で解決できないなら `src/tools/` を修正しろ。
...
2. `src/tools/`, `src/bot-manager/`, `.claude/skills-compact/` を修正
